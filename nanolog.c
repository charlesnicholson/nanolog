...
static void nanolog_extract_and_dispatch(nanolog_binary_field_handler_cb_t cb,
                                         ...
                                         va_list args) {
  switch (type) {
    case NL_VARARG_TYPE_END_OF_LIST: break;
    case NL_VARARG_TYPE_SCALAR_1_BYTE: {
      char const c = (char)va_arg(args, int); cb(ctx, &c, sizeof(c));
    } break;
    case NL_VARARG_TYPE_SCALAR_2_BYTE: {
      short const s = (short)va_arg(args, int); cb(ctx, &s, sizeof(s));
    } break;
    case NL_VARARG_TYPE_SCALAR_4_BYTE: {
      int const i = va_arg(args, int); cb(ctx, &i, sizeof(i));
    } break;
    case NL_VARARG_TYPE_SCALAR_8_BYTE: {
      long long const ll = va_arg(args, long long); cb(ctx, &ll, sizeof(ll));
    } break;
    case NL_VARARG_TYPE_POINTER: {
      void *const v = va_arg(args, void *); cb(ctx, &v, sizeof(v));
    } break;
    case NL_VARARG_TYPE_DOUBLE: {
      double const d = va_arg(args, double); cb(ctx, &d, sizeof(d));
    } break;
    case NL_VARARG_TYPE_LONG_DOUBLE: {
      long double const ld = va_arg(args, long double); cb(ctx, &ld, sizeof(ld));
    } break;
    case NL_VARARG_TYPE_WINT_T: {
      wint_t const w = va_arg(args, wint_t); cb(ctx, &w, sizeof(w));
    } break;
  }
}
