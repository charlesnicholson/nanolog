...
#define INST_TYPE_X_LIST() \
  ...
  X(AND_REG, and_reg, { imm_shift shift; u8 n, m; }) \
  X(AND_IMM, and_imm, { u32 imm; u8 n; }) \
  X(BIT_CLEAR_IMM, bit_clear_imm, { u32 imm; u8 n; }) \
  X(BIT_CLEAR_REG, bit_clear_reg, { imm_shift shift; u8 n, m; }) \
  ...
  X(EXTEND_ADD_SIGNED_BYTE, extend_add_signed_byte, { u8 n, m, rotation; }) \
  X(EXTEND_ADD_SIGNED_HALF, extend_add_signed_half, { u8 n, m, rotation; }) \
  X(EXTEND_ADD_UNSIGNED_BYTE, extend_add_unsigned_byte, { u8 n, m, rotation; }) \
  X(EXTEND_SIGNED_BYTE, extend_signed_byte, { u8 m, rotation; }) \
  X(EXTEND_SIGNED_HALF, extend_signed_half, { u8 m, rotation; }) \
  X(EXTEND_UNSIGNED_BYTE, extend_unsigned_byte, { u8 m, rotation; }) \
  X(EXTEND_UNSIGNED_HALF, extend_unsigned_half, { u8 m, rotation; }) \
  X(EXTEND_ADD_UNSIGNED_HALF, extend_add_unsigned_half, { u8 n, m, rotation; }) \
  X(IF_THEN, if_then, { u8 firstcond, mask, cnt; }) \
  X(LOAD_BYTE_IMM, load_byte_imm, { u16 imm; u8 n, add, index; }) \
  X(LOAD_BYTE_LIT, load_byte_lit, { u16 imm; u8 add; }) \
  X(LOAD_BYTE_REG, load_byte_reg, { imm_shift shift; u8 n, m; }) \
  X(LOAD_DBL_REG, load_dbl_reg, { u16 imm; u8 t, t2, n, index, add; }) \
  X(LOAD_EXCL, load_excl, { u16 imm; u8 n; }) \
  X(LOAD_HALF_IMM, load_half_imm, { u16 imm; u8 n, add, index; }) \
  X(LOAD_HALF_REG, load_half_reg, { imm_shift shift; u8 n, m; }) \
  X(LOAD_SIGNED_BYTE_IMM, load_signed_byte_imm, { u16 imm; u8 n, index, add; }) \
  X(LOAD_SIGNED_BYTE_REG, load_signed_byte_reg, { imm_shift shift; u8 n, m; }) \
  X(LOAD_SIGNED_HALF_IMM, load_signed_half_imm, { u16 imm; u8 n, index, add; }) \
  ...
  X(NOP, nop, {}) \
  X(OR_NOT_REG, or_not_reg, { imm_shift shift; u8 n, m; }) \
  X(OR_IMM, or_imm, { u32 imm; u8 n; }) \
  X(OR_REG, or_reg, { imm_shift shift; u8 n, m; }) \
  ...
  X(SATURATE_UNSIGNED, saturate_unsigned, { imm_shift shift; u8 n, saturate_to; }) \
  X(SELECT_BYTES, select_bytes, { u8 n, m; }) \
